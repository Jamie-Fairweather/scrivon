...
function collectMermaidBlockIds(source: string, tabId: string | null): string[] {
    const prefix = tabId ?? 'none'
    const re = /```\s*mermaid\s*\n([\s\S]*?)```/gi
    return [...source.matchAll(re)].map((_, index) => `${prefix}:${index}`)
}

export function MarkdownPreview({ source, tabId, onExpandBlock, className }: MarkdownPreviewProps) {
    const mermaidBlockIds = useMemo(() => collectMermaidBlockIds(source, tabId), [source, tabId])
    ...
    const components: Components = {
        ...
        code(props: ComponentProps<'code'>) {
            ...
            if (lang === 'mermaid' && isBlock) {
                const index = mermaidBlockIndex++
                const blockId = mermaidBlockIds[index] ?? `${tabId ?? 'none'}:${index}`
