...
export function Empty({ className, ...props }: React.ComponentProps<'div'>): React.ReactElement {
    return (
        <div
            className={cn('flex min-w-0 flex-1 flex-col items-center justify-center gap-6 px-6 py-12 text-center text-balance md:py-20', className)}
            data-slot="empty"
            {...props}
        />
    )
...
export function EmptyTitle({ className, ...props }: React.ComponentProps<'div'>): React.ReactElement {
    return <div className={cn('font-heading text-xl font-semibold', className)} data-slot="empty-title" {...props} />
}
